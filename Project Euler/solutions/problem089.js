...
export default function problem89(n = -1) {
    const parsedRomanNumerals = romanNumerals.split("\n");
    let counter = 0;
    for (let roman of parsedRomanNumerals) {
        let romLen = roman.length;
        let romVal = romanToInt(roman);
        let minRom = intToRoman(romVal);
        let minLen = minRom.length;
        counter += romLen - minLen;
    }
    return counter;
}
...
/* cSpell:disable */
const romanNumerals = `MMMMDCLXXII
MMDCCCLXXXIII
MMMDLXVIIII
MMMMDXCV
DCCCLXXII
