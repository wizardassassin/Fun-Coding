...
export default function problem98(n = -1) {
    ...
    for (const value of arr) {
        ...
        let min = Math.ceil(Math.sqrt(10 ** (len - 1)));
        let max = Math.floor(Math.sqrt(10 ** len));
        for (let i = min; i < max; i++) {
            let str = String(i);
            let str1 = String(i ** 2);
            let len1 = str1.length;
            if (new Set(str1).size != len1) {
                continue;
            }
            ...
            for (let a = 0; a < len2; a++) {
                ...
                let map = new Map();
                for (let j = 0; j < len; j++) {
                    map.set(word[j], str1[j]);
                }
                for (let b = a + 1; b < len2; b++) {
                    let w = value[b];
                    let num = "";
                    for (let char of w) {
                        num += map.get(char);
